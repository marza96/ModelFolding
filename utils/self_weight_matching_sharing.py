...
def merge_channel_p_name_clustering(perm_to_axes, params, p_name, merge, indices, scales):
        ...
        if wk == "conv1.weightdd" or wk == "conv1.bndd" in wk:
            ...
            if axis == 0:
                ...
                if len(p.shape) == 1:
                    ...
                    sh = p.shape
                    merged =  m @ p.clone().detach().reshape(sh[0], -1)
                    merged = merged.reshape(m.shape[0], *sh[1:])
                    params[wk] = merged
                ...
                if "classifier.0" in wk:
                    ...
                    sh = p.shape
                    merged =  merge @ torch.diag(scales) @ p.clone().detach().reshape(sh[0], -1)
                    merged = merged.reshape(merge.shape[0], *sh[1:])
...
def self_merge_weight_clustering(perm_to_axes, params, max_ratio=0.5, threshold=0.1, hooks=None):
    ...
    for p_name in perm_sizes.keys():
        ...
        merger = KMeansWM(int(distance.shape[0] * max_ratio), distance.shape[0], normalize=False)
        merge, inertia = merger(distance)
        merges[p_name] = merge
        
        distance = distance.to("cuda")
        means = (torch.diag(1.0 / torch.diag(merge @ merge.T))) @ merge @ distance

        scales[p_name] = torch.norm(distance, dim=1) / torch.norm(merge.T @ means, dim=1)
        closest = list()
        for i in range(merge.shape[0]):
            ...
            for j in range(merge.shape[1]):
                ...
                dist = torch.norm(distance[j, :] - means[i, :]) ** 2
                if dist < cl_dist:
                    cl_dist = dist
                    cl_idx = j
            ...
            closest.append(cl_idx)
        
        indices[p_name] = torch.tensor(closest)

    for p_name in perm_sizes.keys():
        merge = merges[p_name]
        prm = merge_channel_p_name_clustering(perm_to_axes, params, p_name, merge, indices[p_name], scales[p_name])
